      ...
      <div className="container mx-auto px-4 md:px-6 max-w-6xl">
        ...
        <div className="mb-8 space-y-2">
          <h1 className="text-3xl md:text-4xl font-black text-neutral-1">
            {property?.title ? `Reviews for ${property.title}` : 'Guest Reviews'}
          </h1>
          {total > 0 && (
        ...
        {!isLoading && reviews.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
            ...
            <p className="text-neutral-2 text-sm">Be the first to leave a review for this property.</p>
          </div>
        )}

        {/* Reviews list */}
        {!isLoading && reviews.length > 0 && (
          <div className="space-y-0 divide-y divide-gray-100 border border-gray-100 rounded-2xl overflow-hidden bg-white">
            ...
            <div className="flex items-center gap-2">
              {Array.from({ length: totalPage }).map((_, i) => (
                <button
                  key={i}
                  onClick={() => setPage(i + 1)}
                  className={`w-9 h-9 rounded-full text-sm font-medium transition-all cursor-pointer ${i + 1 === page
