...
export default function HotelsPage() {
  ...
  const hasActiveFilters = searchInput || priceRange[0] > 0 || priceRange[1] < MAX_PRICE || ratingFilter.length > 0 || availableOnly;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-4 md:px-6 py-12 md:pb-24 lg:flex gap-12">
        ...
        {/* Mobile filter toggle */}
        <button onClick={() => setShowFilters(!showFilters)} className="lg:hidden w-full mb-8 h-12 px-5 bg-white border border-gray-100 rounded-xl flex items-center justify-between font-bold text-neutral-1 active:scale-95 transition-all">
        ...
        <aside className={`${showFilters ? 'block' : 'hidden'} lg:block lg:w-[300px] xl:w-[320px] flex-shrink-0 space-y-6 mb-8 lg:mb-0`}>
          <div className="bg-white rounded-2xl border border-gray-100 p-6 sm:p-8 space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-neutral-1 text-sm sm:text-base">{t('listing.sidebar.price_range')}</h3>
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 transition-opacity ${isFetching ? 'opacity-60' : 'opacity-100'}`}>
              {hotels.map((hotel: any, index: number) => (
                <motion.div key={hotel._id ?? hotel.id ?? index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }}>
                  <Link href={`/hotels/${hotel._id ?? hotel.id}`} className="group block bg-white rounded-sm overflow-hidden border border-gray-100 hover:shadow-sm transition-all duration-500">
                    <div className="relative aspect-[16/10] overflow-hidden">
                      ...
                      {hotel.status && <span className="absolute top-4 left-4 bg-primary text-white text-[10px] font-bold px-3 py-1.5 rounded-full shadow-sm capitalize">{hotel.status.replace('_', ' ')}</span>}
                      <div className="absolute bottom-4 right-4 flex gap-2 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                        <button className="w-10 h-10 rounded-sm bg-white/90 backdrop-blur flex items-center justify-center text-neutral-1 hover:bg-primary hover:text-white transition-all shadow-lg"><Bookmark size={18} /></button>
                        <button className="w-10 h-10 rounded-sm bg-white/90 backdrop-blur flex items-center justify-center text-neutral-1 hover:bg-primary hover:text-white transition-all shadow-lg"><Search size={18} /></button>
                      </div>
                    </div>
                    <div className="p-4 sm:p-5 space-y-4">
                      <div className="space-y-3">
                        <h3 className="text-xl sm:text-2xl font-bold text-neutral-1 group-hover:text-primary transition-colors line-clamp-1">{hotel.name ?? hotel.title}</h3>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                        ...
                        <Button className="bg-[#F1913D] hover:bg-[#F1913D]/90 text-white font-medium h-10 sm:h-12 px-4 sm:px-6 rounded-sm transition-all active:scale-95 text-sm sm:text-base border-none">{t('hotels.main.book_now')}</Button>
                      </div>
                    </div>
                  </Link>
                </motion.div>
